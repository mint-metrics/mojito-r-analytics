  ...
  if(!is.na(segment_type)) {
      segment_clause <- paste0("
          AND x.subject ",ifelse(segment_negative, "NOT","")," IN (
            ...
            FROM mojito.segments_",wave_params$subject,"
  ...
  query <- paste0(
    ...
    WITH
      ...
      user_goal_count AS (
        SELECT 
          c.subject
        FROM mojito.",wave_params$client_id,"_conversions_",wave_params$subject," c
          INNER JOIN mojito.exposures_",wave_params$subject," x
      ...
      daily_aggregate AS (
        ...
        FROM mojito.exposures_",wave_params$subject," x
...
mojitoGetUniqueTrafficConversions <- function(wave_params, goal, operand="=", goal_count=1, segment_type=NA, segment_value=NA) {
  ...
  query <- paste0(
    ...
    FROM mojito.exposures_usercookie x
    INNER JOIN snowplow_intermediary.traffic_sources_1 t
    ...
    WHERE exposure_time BETWEEN convert_timezone('",mojitoReportTimezone,"', 'UTC', '",wave_params$start_date,"')
      and convert_timezone('",mojitoReportTimezone,"', 'UTC', '",wave_params$stop_date,"')
    AND client_id = '",wave_params$client_id,"'
    AND wave_id = '",wave_params$wave_id,"'
    AND NOT (x.subject IS NULL OR x.recipe_name IS NULL)
...
mojitoGetConversionTimeIntervals <- function(wave_params, goal, operand="=", time_grain="minute", max_interval=30) {
  ...
  query <- paste0(
    ...
    FROM mojito.exposures_",wave_params$subject," x
      INNER JOIN mojito.",wave_params$client_id,"_conversions_",wave_params$subject," c
        ON (
        x.subject = c.subject
        AND x.exposure_time < c.conversion_time
        AND exposure_time BETWEEN
...
mojitoGetRevenueOrders <- function(wave_params, goal, operand="=", segment=NA, segment_val_operand="=", segment_negative=FALSE) {
  ...
  query <- paste0(
    ...
    WITH daily_aggregate AS (
        ...
        FROM mojito.exposures_",wave_params$subject," x
        LEFT JOIN mojito.",wave_params$client_id,"_conversions_",wave_params$subject," c
        ON (
            x.subject = c.subject
            AND x.exposure_time < c.conversion_time
            AND exposure_time BETWEEN 
...
mojitoGetErrorsChart <- function(wave_params) {
  query <<- paste0("
    SELECT 
            component,
            DATE_TRUNC('",wave_params$time_grain,"', convert_timezone('UTC', 'Australia/Melbourne', derived_tstamp)) AS tstamp,
            COUNT(*) AS errors,
            count(distinct subject) AS subjects
    FROM mojito.recipe_errors_2
